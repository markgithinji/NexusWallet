...
class GetBitcoinDetailUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        logger.d(tag, "Getting Bitcoin details for wallet: $walletId, network: ${network.displayName}")
        ...
        val bitcoinCoin = wallet.bitcoinCoins.find { it.network == network }
            ...
            ?: return Result.Error("Bitcoin ${network.displayName} not enabled for this wallet")
        ...
        when (txResult) {
            is Result.Success -> {
                ...
                txResult.data.forEachIndexed { index, tx ->
                    ...
                    logger.d(tag, "Saved transaction $index: ${tx.txHash?.take(8) ?: "unknown"} on ${network.displayName}")
                }
        ...
        val result = BitcoinDetailResult(
            ...
            network = bitcoinCoin.network,
            networkDisplayName = bitcoinCoin.network.displayName,
            rawTransactions = rawTransactions,
            bitcoinCoin = bitcoinCoin,
            availableNetworks = wallet.bitcoinCoins.map { it.network }
        )

        logger.d(tag, "=== GetBitcoinDetailUseCase completed successfully with ${rawTransactions.size} raw transactions on ${network.displayName} ===")
